...
let alltext;
/*
function offsetPlatforms(objArr){
    objArr.forEach(obj => {
        let maxPlat = 0;
...
function level1(){
    ...
    items = items.concat(fixOldOffsetLevels(tempitems));
    platforms = platforms.concat(fixOldOffsetLevels(tempplatforms));
    calcWinx(platforms);
}
// hi C:D
function level2(){
...
function level3(){
    ...
    tempitems = [
        new Item(width + width*0.69, height*0.6, coin, innerWidth/48, innerWidth/48),
        new Item(width + width*0.94, height*0.4, coin, innerWidth/48, innerWidth/48),
        
        new Item(width + width*1.05, height*0.25, coin, innerWidth/48, innerWidth/48),
        new Item(width + width*1.4, height*0.3, coin, innerWidth/48, innerWidth/48),

        new Item(width + width*1.79, height*0.64, coin, innerWidth/48, innerWidth/48),        
        new Item(width + width*2.1, height*0.5, coin, innerWidth/48, innerWidth/48),
        new Item(width + width*2.4, height*0.74, coin, innerWidth/48, innerWidth/48),
    ];
    items = tempitems;//items.concat(fixOldOffsetLevels(tempitems));
    platforms = platforms.concat(fixOldOffsetLevels(tempplatforms));
    calcWinx(platforms);
}
...
function randomGen(){// test
    ...
    */
    ...
    if(Math.random() > 0.5){
        ...
        tempplatforms[0] = new Platform(0, height*0.8, width*0.5, height*0.2);
        for(let i = 1; i < 9; i++){
            ...
            tempplatforms[i] = new Platform(width * platpos.x, height * platpos.y, width*platdim.x, height*platdim.y);

            //COINS man
            if(difficulty != 'run'){
                for(let j = 0; j < Math.floor(Math.random()*5); j++){
                    coinpos.x = Math.random() * (tempplatforms[i].width+tempplatforms[i].position.x - tempplatforms[i].position.x) + tempplatforms[i].position.x;
                    coinpos.y = Math.random() * (tempplatforms[i].position.y-0.06*height - (tempplatforms[i].position.y-0.3*height)) +  (tempplatforms[i].position.y-0.3*height);
                    tempitems[i+j] = new Item(coinpos.x, coinpos.y, coin, innerWidth/48, innerWidth/48);
                }
            }
        }
        tempplatforms[9] = new Platform(platforms[8].position.x+platforms[8].width + (Math.random()*0.3), height*0.8, width*0.5, height*0.2);
        ...
        for(let i = 1; i < 9; i++){
            ...
            if(difficulty != 'run' && Math.random() > 0.5){
                coinpos.x = Math.random() * (tempplatforms[i].width+tempplatforms[i].position.x - tempplatforms[i].position.x) + tempplatforms[i].position.x;
                coinpos.y = Math.random() * (tempplatforms[i].position.y-0.06*height - (tempplatforms[i].position.y-0.3*height)) +  (tempplatforms[i].position.y-0.3*height);
                tempitems[i] = new Item(coinpos.x, coinpos.y, coin, innerWidth/48, innerWidth/48);
            } 
        }
        tempplatforms[9] = new Platform(tempplatforms[8].position.x+tempplatforms[8].width + (Math.random()*0.3), height*0.8, width*0.5, height*0.2);
...
function draw(){
    if (!dead && !menu && coinload && !stoped) requestAnimationFrame(draw);
...
let clickcount = 0;
function update(){
    if (!dead && !menu && coinload && !stoped) setTimeout(update, 1000/fps);
...
function printScores(){
    ...
    */
    ...
    document.getElementById('b').innerHTML = "Win: " + ratioWin;
    
    //suppa debugging :thumbs:
    document.getElementById('c').innerHTML = "Distance: " + distance;
    document.getElementById('d').innerHTML = "Win: " + winx;
    document.getElementById('e').innerHTML = "scrollOffset: " + scrollOffset;
    document.getElementById('e').innerHTML = "Last Plat: " + platforms[platforms.length-1].position.x;
